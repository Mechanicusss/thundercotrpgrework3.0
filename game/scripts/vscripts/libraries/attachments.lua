...
LinkLuaModifier("modifier_animation_freeze_stun", "libraries/attachments", LUA_MODIFIER_MOTION_NONE )
...
function Attachments:ActivateAttachmentSetup()
  ...
  --[[if addon == nil or addon == "" then
    print("[Attachments] Addon name must be specified.")
    return
  end]]

  if not io then
    print("[Attachments] Attachments Setup is only available in tools mode.")
    return
  end
  if not Attachments.activated then
    ...
    if not file and Attachments.dbFilePath == nil then
      print("[Attachments] Cannot find file 'dota_addons/" .. addon .. "/scripts/attachments.txt'.  Re-execute the console command to force create the file.")
    ...
    if not file then
      ...
      print("[Attachments] Created file: 'dota_addons/" .. addon .. "/scripts/attachments.txt'.")
    end
    file:close()
...
function Attachments:Attachment_SaveAttach(args)
  ...
  if Attachments.dbFilePath == nil or Attachments.dbFilePath == "" then
    print("[Attachments] Attachments database file must be set.")
    return
  end
...
function Attachments:AttachProp(unit, attachPoint, model, scale, properties)
    ...
    if not properties then
      if not db[unitModel] or not db[unitModel][attachPoint] or not db[unitModel][attachPoint][propModel] then
        print("[Attachments] No attach found in attachment database for '" .. unitModel .. "', '" .. attachPoint .. "', '" .. propModel .. "'")
        return
      end
    end
