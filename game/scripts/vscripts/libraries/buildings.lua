...
LinkLuaModifier("modifier_building_construction", "libraries/buildings", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_building_hide_on_minimap", "libraries/buildings", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_building_health", "libraries/buildings", LUA_MODIFIER_MOTION_NONE)
...
function FindCustomBuildingsInRadius(position, radius)
    ...
    -- Находим все сущности в заданном радиусе
    local candidates = Entities:FindAllByClassnameWithin("npc_dota_creature", position, radius)

    local custom_buildings = {}

    -- Проходим по всем найденным сущностям
    for _, creature in pairs(candidates) do
        -- Проверяем, существует ли метод IsCustomBuilding
        if creature and creature:IsCustomBuilding() then
            -- Проверяем, есть ли у сущности модель
            if creature:GetModel() and creature:GetModel() ~= "" then
                table.insert(custom_buildings, creature)
            else
                print("Сущность не имеет модели: ", creature:GetUnitName())
            end
        end
    end
