...
function PlayerTables:CreateTable(tableName, tableContents, pids)
  ...
  if self.tables[tableName] then
    print("[playertables] Warning: player table '" .. tableName .. "' already exists.  Overriding.")
  end
  ...
  for k,v in pairs(pids) do
    ...
    if pid >= 0 and pid < DOTA_MAX_TEAM_PLAYERS then
      ...
      print("[playertables] Warning: Pid value '" .. pid .. "' is not an integer between [0," .. DOTA_MAX_TEAM_PLAYERS .. "].  Ignoring.")
    end
  end
end
...
function PlayerTables:DeleteTable(tableName)
  if not self.tables[tableName] then
    print("[playertables] Warning: Table '" .. tableName .. "' does not exist.")
    return
  end
...
function PlayerTables:AddPlayerSubscription(tableName, pid)
  ...
  if not oldPids[pid] then
    if pid >= 0 and pid < DOTA_MAX_TEAM_PLAYERS then
      ...
      print("[playertables] Warning: Pid value '" .. v .. "' is not an integer between [0," .. DOTA_MAX_TEAM_PLAYERS .. "].  Ignoring.")
    end
  end
end
