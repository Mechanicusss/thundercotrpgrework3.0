...
if IsServer() then
	...
	function modifier_arthas_vsolyanova_active:OnAttackLanded(keys)
		if keys.attacker == self:GetParent() then
			...
			local chance = ability:GetAbilityValues(attacker:IsIllusion() and "nia_chance_illusions" or "nia_chance") * (attacker:GetTalentSpecial("talent_hero_arthas_vsolyanova_bunus_chance", "chance_multiplier") or 1)
