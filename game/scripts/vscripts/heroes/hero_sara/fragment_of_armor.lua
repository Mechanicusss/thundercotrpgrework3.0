...
LinkLuaModifier("modifier_sara_fragment_of_armor", "heroes/hero_sara/fragment_of_armor", LUA_MODIFIER_MOTION_NONE)
...
if IsServer() then
	function modifier_sara_fragment_of_armor:OnTakeDamage(keys)
		...
		if unit == keys.unit and
			...
			unit:GetEnergy() >= (keys.damage * ability:GetAbilityValues("blocked_damage_pct") * 0.01) / ability:GetAbilityValues("damage_per_energy") then
