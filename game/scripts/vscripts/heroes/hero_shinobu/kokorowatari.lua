function CreateSoul(keys)
	...
	if not target:IsHero() and
		...
		soul.DeathTime = GameRules:GetGameTime() + ability:GetAbilityValues("soul_duration")
	end
end
...
function ChanceToKill(keys)
	...
	local ability = keys.ability
	if not target:IsConsideredHero() and not target:IsMagicImmune() and not target:IsInvulnerable() and RollPercentage(ability:GetAbilityValues("chance_to_kill")) then
