function StealHealth(keys)
	...
	local pct = ability:GetAbilityValues("lifesteal_pct_lvl" .. ability.CurrentLevel)
	if pct then
...
function UpdateStateLevel(keys)
	...
	if IsValidEntity(ability) then
		...
		local mark_lvl2 = ability:GetAbilityValues("hp_mark_pct_lvl2")
		local mark_lvl3 = ability:GetAbilityValues("hp_mark_pct_lvl3")
		local mark_lvl4 = ability:GetAbilityValues("hp_mark_pct_lvl4")
		local lvl = 1
