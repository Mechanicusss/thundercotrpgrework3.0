function Superposition(keys)
	...
	local distance = ability:GetAbilityValues("distance")
	...
	local illusion_duration = ability:GetAbilityValues("illusion_duration")
	local illusion = Illusions:create({
		unit = caster,
		damageIncoming = ability:GetAbilityValues("illusion_damage_percent_incoming"),
		damageOutgoing = ability:GetAbilityValues("illusion_damage_percent_outgoing"),
	...
	Timers:NextTick(function()
		...
		local channel_time = math.min(illusion_duration, illusion_ability:GetAbilityValues("channel_time") - (GameRules:GetGameTime() - ability.LastInterruptedAbilityCastTime))
		if channel_time > 0 then
			...
			elseif name == "zen_gehraz_mystic_twister" then
				...
				ParticleManager:SetParticleControl(illusion_ability.thinker_dummy.particle, 1, Vector(illusion_ability:GetAbilityValues("radius")))
