function IncreaseAbilityStacks(unit, ability)
	...
	ability.AdditionalModelScale = math.min(ability:GetAbilityValues("model_scale_per_stack") * unit:GetModifierStackCount("modifier_pudge_flesh_heap_arena_stack", unit), ability:GetAbilityValues("model_scale_max"))
...
function OnKill(keys)
	...
	if unit:IsRealCreep() then
		...
		local creeps_killed_to_stack = ability:GetAbilityValues("creeps_killed_to_stack")
