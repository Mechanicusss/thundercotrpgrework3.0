...
item_summoning_scroll_spider = class(ItemBaseClass)

BOSS_NAME = "npc_dota_creature_40_boss"
-------------
function item_summoning_scroll_spider:OnSpellStart()
    if not IsServer() then return end

    local caster = self:GetCaster()

    if caster:GetLevel() < 30 then
        DisplayError(caster:GetPlayerID(), "Level Too Low To Summon. Requires Level " .. 30 .. ".")
