...
item_summoning_scroll_skafian = class(ItemBaseClass)

BOSS_NAME = "npc_dota_creature_30_boss"
-------------
function item_summoning_scroll_skafian:OnSpellStart()
    if not IsServer() then return end

    local caster = self:GetCaster()

    if caster:GetLevel() < 15 then
        DisplayError(caster:GetPlayerID(), "Level Too Low To Summon. Requires Level " .. 15 .. ".")
