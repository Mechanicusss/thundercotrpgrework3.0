...
item_summoning_scroll_roshan = class(ItemBaseClass)

BOSS_NAME = "npc_dota_creature_roshan_boss"
-------------
function item_summoning_scroll_roshan:OnSpellStart()
    if not IsServer() then return end

    local caster = self:GetCaster()

    if caster:GetLevel() < 10 then
        DisplayError(caster:GetPlayerID(), "Level Too Low To Summon. Requires Level " .. 10 .. ".")
    ...
    CreateUnitByNameAsync(BOSS_NAME, caster:GetAbsOrigin(), true, nil, nil, DOTA_TEAM_NEUTRALS, function(unit)
        ...
        unit:AddNewModifier(unit, nil, "modifier_boss_roshan", {
