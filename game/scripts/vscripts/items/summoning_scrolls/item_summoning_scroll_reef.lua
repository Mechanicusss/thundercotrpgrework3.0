...
item_summoning_scroll_reef = class(ItemBaseClass)

BOSS_NAME = "npc_dota_creature_50_boss"
-------------
function item_summoning_scroll_reef:OnSpellStart()
    if not IsServer() then return end

    local caster = self:GetCaster()

    if caster:GetLevel() < 50 then
        DisplayError(caster:GetPlayerID(), "Level Too Low To Summon. Requires Level " .. 50 .. ".")
