...
item_summoning_scroll_zeus = class(ItemBaseClass)

BOSS_NAME = "npc_dota_creature_150_boss_last"
-------------
function item_summoning_scroll_zeus:OnSpellStart()
    if not IsServer() then return end

    local caster = self:GetCaster()

    if caster:GetLevel() < MAX_LEVEL then
        DisplayError(caster:GetPlayerID(), "Level Too Low To Summon. Requires Level " .. MAX_LEVEL .. ".")
