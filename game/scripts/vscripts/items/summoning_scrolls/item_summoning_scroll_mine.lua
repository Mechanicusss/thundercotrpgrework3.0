...
item_summoning_scroll_mine = class(ItemBaseClass)

BOSS_NAME = "npc_dota_creature_70_boss"
-------------
function item_summoning_scroll_mine:OnSpellStart()
    if not IsServer() then return end

    local caster = self:GetCaster()

    if caster:GetLevel() < 75 then
        DisplayError(caster:GetPlayerID(), "Level Too Low To Summon. Requires Level " .. 75 .. ".")
