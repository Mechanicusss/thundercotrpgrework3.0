...
function aghanim_summon_portals:OnAbilityPhaseStart()
	if IsServer() then
		...
		local damageDifficultyMultiplier = 1 
		if mode == "HARDCORE" then
			damageDifficultyMultiplier = DIFFICULTY_BOSS_AGHANIM_DAMAGE_MULTIPLIER_HARDCORE
		elseif mode == "HARD" then
			damageDifficultyMultiplier = DIFFICULTY_BOSS_AGHANIM_DAMAGE_MULTIPLIER_HARD
		elseif mode == "IMPOSSIBLE" then
			damageDifficultyMultiplier = DIFFICULTY_BOSS_AGHANIM_DAMAGE_MULTIPLIER_IMPOSSIBLE
		elseif mode == "HELL" then
			damageDifficultyMultiplier = DIFFICULTY_BOSS_AGHANIM_DAMAGE_MULTIPLIER_HELL
		end

		self.staff_crush_damage = self.staff_crush_damage * damageDifficultyMultiplier
...
function aghanim_summon_portals:OnSpellStart()
	if IsServer() then
		...
		if self.nMode == self.PORTAL_MODE_ALL_SPEARS then
			...
			local vLineDir = nil
			--self.Heroes = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetCaster():GetAbsOrigin(), nil, self:GetCaster():GetAcquisitionRange(), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS, FIND_CLOSEST, false )
			self.Heroes = GetVisibleEnemyHeroesInRange(self:GetCaster(), self:GetCaster():GetAcquisitionRange())
...
function aghanim_summon_portals:CreateNearHeroPortal()
	if IsServer() then
		...
		local Heroes = GetVisibleEnemyHeroesInRange(self:GetCaster(), self:GetCaster():GetAcquisitionRange())
		if #Heroes == 0 then
			return
		end
