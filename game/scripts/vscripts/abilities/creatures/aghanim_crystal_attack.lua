...
_G.CRYSTAL_ATTACKS_COOLDOWN = 1.0
...
function aghanim_crystal_attack:OnSpellStart()
	if IsServer() then
		...
		self.attack_damage = self:GetCaster():GetAverageTrueAttackDamage(self:GetCaster())
		self.explosion_radius = self:GetSpecialValueFor( "explosion_radius" ) 
		self.pulse_radius = self:GetSpecialValueFor( "pulse_radius" ) 

		--local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetCaster():GetAbsOrigin(), nil, self:GetCaster():GetAcquisitionRange(), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS, FIND_FARTHEST, false )
		local enemies = GetVisibleEnemyHeroesInRange(self:GetCaster(), self:GetCaster():GetAcquisitionRange())
