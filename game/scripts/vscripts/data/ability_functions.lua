...
local function OctarineLifesteal(attacker, victim, inflictor, damage, damagetype_const, itemname, cooldownModifierName)
	if inflictor and attacker:GetTeam() ~= victim:GetTeam() and not OCTARINE_NOT_LIFESTALABLE_ABILITIES[inflictor:GetAbilityName()] then
		local heal = math.floor(damage * GetAbilityValues(itemname, victim:IsHero() and "hero_lifesteal" or "creep_lifesteal") * 0.01)
		if heal >= 1 then
		...
		if item and RollPercentage(GetAbilityValues(itemname, "bash_chance")) and not attacker:HasModifier(cooldownModifierName) then
			victim:AddNewModifier(attacker, item, "modifier_stunned", {duration = GetAbilityValues(itemname, "bash_duration")})
...
ON_DAMAGE_MODIFIER_PROCS_VICTIM = {
	modifier_item_holy_knight_shield = function(attacker, victim, inflictor, damage) if inflictor then
		...
		if item and RollPercentage(GetAbilityValues("item_holy_knight_shield", "buff_chance")) and victim:GetTeam() ~= attacker:GetTeam() then
...
OUTGOING_DAMAGE_MODIFIERS = {
		...
		multiplier = function(attacker, victim, _, damage)
			...
			if kadash_strike_from_shadows then
				ApplyDamage({
					victim = victim,
					attacker = attacker,
					damage = damage * kadash_strike_from_shadows:GetAbilityValues("magical_damage_pct") * 0.01,
	...
	modifier_item_piercing_blade = {
		...
		multiplier = function()
			return 1 - GetAbilityValues("item_piercing_blade", "attack_damage_to_pure_pct") * 0.01
		end
	},
	modifier_item_soulcutter = {
		...
		multiplier = function()
			return 1 - GetAbilityValues("item_soulcutter", "attack_damage_to_pure_pct") * 0.01
		end
	},
	...
	modifier_item_golden_eagle_relic = function(_, _, inflictor)
		if not IsValidEntity(inflictor) then
			return {
				LifestealPercentage = GetAbilityValues("item_golden_eagle_relic", "lifesteal_pct")
			}
		end
	end,
...
INCOMING_DAMAGE_MODIFIERS = {
	modifier_mirratie_sixth_sense = {
		multiplier = function(_, victim)
			...
			if mirratie_sixth_sense and victim:IsAlive() and RollPercentage(mirratie_sixth_sense:GetAbilityValues("dodge_chance_pct")) and not victim:PassivesDisabled() then
	...
	modifier_item_blade_mail_arena_active = {
		multiplier = function(_, victim)
			...
			if modifier and IsValidEntity(modifier:GetAbility()) then
				return 1 - modifier:GetAbility():GetAbilityValues("reduced_damage_pct") * 0.01
			end
		end
	},
	modifier_item_sacred_blade_mail_active = {
		multiplier = function()
			return 1 - GetAbilityValues("item_sacred_blade_mail", "reduced_damage_pct") * 0.01
		end
	},
		...
		multiplier = function(attacker, victim, inflictor, damage)
			...
			if not IsValidEntity(inflictor) and saber_instinct and victim:IsAlive() and not victim:PassivesDisabled() then
				if attacker:IsRangedUnit() then
					if RollPercentage(saber_instinct:GetAbilityValues("ranged_evasion_pct")) then
					...
					if RollPercentage(saber_instinct:GetAbilityValues("melee_block_chance")) then
						local blockPct = saber_instinct:GetAbilityValues("melee_damage_pct") * 0.01
						return {
	...
	modifier_sara_fragment_of_armor = {
		multiplier = function(attacker, victim, inflictor, damage)
			...
			if sara_fragment_of_armor and not victim:IsIllusion() and victim:IsAlive() and not victim:PassivesDisabled() and victim.GetEnergy and sara_fragment_of_armor:GetToggleState() then
				local blocked_damage_pct = sara_fragment_of_armor:GetAbilityValues("blocked_damage_pct") * 0.01
				local mana_needed = (damage * blocked_damage_pct) / sara_fragment_of_armor:GetAbilityValues("damage_per_energy")
	...
	modifier_anakim_transfer_pain = {
		multiplier = function(attacker, victim, inflictor, damage)
			...
			if anakim_transfer_pain and victim:IsAlive() then
				local transfered_damage_pct = anakim_transfer_pain:GetAbilityValues("transfered_damage_pct")
				local radius = anakim_transfer_pain:GetAbilityValues("radius")
	...
	modifier_item_timelords_butterfly = {
		multiplier = function(_, victim)
			if victim:IsAlive() and not victim:IsMuted() and RollPercentage(GetAbilityValues("item_timelords_butterfly", "dodge_chance_pct")) then
				ParticleManager:CreateParticle("particles/units/heroes/hero_faceless_void/faceless_void_backtrack.vpcf", PATTACH_ABSORIGIN_FOLLOW, victim)
				return false
			end
		end
...
CREEP_BONUSES_MODIFIERS = {
	modifier_item_golden_eagle_relic = {
		gold = GetAbilityValues("item_golden_eagle_relic", "kill_gold"),
		xp = GetAbilityValues("item_golden_eagle_relic", "kill_xp")
	},

	modifier_item_skull_of_midas = {
		gold = GetAbilityValues("item_skull_of_midas", "kill_gold"),
		xp = GetAbilityValues("item_skull_of_midas", "kill_xp")
	},
